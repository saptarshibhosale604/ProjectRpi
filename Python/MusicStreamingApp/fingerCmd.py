...
def updateFile(data):
	with open(mode_file, "w") as file:
		file.write(data)  # Update mode in file	
...
while True:
	...
	if len(b and a)!=0:
		...
		# If no hand is detected, reset values
		up = 0
		down = 0
	...
	# Call Cmd only if a hand is detected
	if up > 0:
		Cmd(up)
