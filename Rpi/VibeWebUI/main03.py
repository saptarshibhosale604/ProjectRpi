...
The project now supports **two independent rendering modes**:

1. **UI Generation**

   * The user enters a prompt.
   * The LLM generates a JSON UI description.
   * Python converts the JSON into HTML components.
   * Components are streamed to the browser in real time.

2. **SVG Image Generation**

   * The user enters a prompt.
   * The LLM generates a pure SVG document.
   * The server extracts and validates the SVG.
   * The browser directly renders the generated vector image.
...
* Live JSON UI generation
* Incremental streaming using Server-Sent Events (SSE)
* SVG image generation from natural language prompts
* Modular Ollama communication functions
* Automatic logging
...
The LLM streams JSON token-by-token.

As tokens arrive:

1. Every token is logged with a `[TOKEN]` prefix.
2. The accumulated JSON is scanned for complete child objects.
3. Completed components are immediately rendered.
4. The browser updates without waiting for the full response.
...
The SVG generator uses a dedicated communication function:
...
* Build the SVG-specific prompt
* Stream tokens from Ollama
* Log every token
* Accumulate the response
* Extract the `<svg>...</svg>` block
* Return the SVG to the `/svg` endpoint

Unlike JSON mode, SVG generation waits for the complete response before rendering.

---

# Web Interface

The application provides two actions:
...
which are displayed directly in the browser.
...
[INFO] Streaming component #0

[DEBUG] Full LLM output ...
```
...
```
[INFO] SVG Query → Ollama

[DEBUG] [SVG TOKEN] ...

[DEBUG] Full SVG response ...

[INFO] SVG extracted successfully
```

Errors are also logged with stack traces when applicable.

---

# Configuration

Modify the `CONFIG` dictionary to change:
...
* JSON mode expects valid JSON only.
* SVG mode expects valid SVG only.
* Tokens are streamed and logged for debugging.
* UI components are rendered incrementally.
* SVG output is extracted from the model response before being sent to the browser.
* Errors are displayed in the browser instead of crashing the server.
...
# ---------------------------------------------------------------------------
# Ollama SVG generator
# ---------------------------------------------------------------------------

def StreamOllamaSVG(userQuery: str) -> str:
    """
    Sends an SVG generation request to Ollama and returns the generated SVG.
...
"""
    ...
    logger.info("SVG Query → Ollama: %s", userQuery[:120])
    ...
    try:
        ...
        with requests.post(
            ...
            for rawLine in resp.iter_lines():
                ...
                if token:
                    ...
                    logger.debug("[SVG TOKEN] %s", repr(token))
        ...
        logger.error("SVG generation timed out")
        raise

    except Exception as exc:

        logger.exception("SVG generation failed: %s", exc)
        raise

    logger.debug(
        "Full SVG response (%d chars):\n%s",
        len(accumulated),
        accumulated,
    )

    # Extract only the SVG portion if extra text exists
    start = accumulated.find("<svg")
    end = accumulated.rfind("</svg>")

    if start != -1 and end != -1:
        svg = accumulated[start:end + len("</svg>")]
...
function GenerateImage() {
    ...
    fetch("/svg?q=" + encodeURIComponent(query))
    .then(r => r.text())
    .then(svg => {
        ...
        statusEl.textContent = "✓ SVG generated";
...
def Svg():
    ...
    svg = StreamOllamaSVG(userQuery)

    logger.info("Sending SVG to browser (%d chars)", len(svg))
